...
- **Test Yourself Link**: The Test Yourself link is the sole exception to the Wikilink rule. Use `quiz=<URL-encoded vault-relative worksheet path>` as its Markdown destination. URL-encode the entire path (including `/`) and never use a vault name or an absolute filesystem path. Add the same generated-file link once to the worksheet and once to the source explanation note; avoid duplicates.
