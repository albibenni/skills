...
3. **Write Compact Items**: State the decision point first. Aim for one sentence per question; use a second short sentence only when it supplies essential scenario context. Keep each option to one line and roughly the same length.
...
- **Plausible, Balanced Distractors**: Base wrong options on realistic misconceptions or almost-correct alternatives. Keep all four options parallel in grammar, detail, and approximate length. Do not signal the answer through precision, qualifiers, or a visibly longer explanation. Randomly vary correct letters across A–D; avoid predictable sequences.
